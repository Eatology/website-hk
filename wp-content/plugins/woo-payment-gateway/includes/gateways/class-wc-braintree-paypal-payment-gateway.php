...
class WC_Braintree_PayPal_Payment_Gateway extends WC_Braintree_Payment_Gateway {
			...
			'patch' => array(
				array(
					...
					'value' => array(
						...
						'value'         => wc_format_decimal( round( WC()->cart->total, $decimals ), $decimals ),
						'breakdown'     => array(
							'item_total'        => array(
								'currency_code' => $currency,
								'value'         => wc_format_decimal( round( WC()->cart->subtotal_ex_tax + WC()->cart->fee_total, $decimals ), $decimals )
							),
							'shipping'          => array(
								'currency_code' => $currency,
								'value'         => wc_format_decimal( round( WC()->cart->get_shipping_total(), $decimals ), $decimals )
							),
							'tax_total'         => array(
								'currency_code' => $currency,
								'value'         => wc_format_decimal( round( WC()->cart->get_taxes_total(), $decimals ), $decimals )
							),
							'shipping_discount' => array(
								'currency_code' => $currency,
								'value'         => wc_format_decimal( round( WC()->cart->discount_cart, $decimals ), $decimals )
							)
						)
					)
				),
	...
	public function get_formatted_shipping_method( $method, $index, $selected, $amount, $decimals ) {
		...
		return array(
			...
			'selected' => $selected,
			'amount'   => array(
				'value'         => wc_format_decimal( round( $method->cost, $decimals ), $decimals ),
	...
	private function can_show_bnpl_msg() {
		...
		if ( ! is_admin() && ! in_array( get_woocommerce_currency(), array( 'USD', 'GBP' ) ) ) {
			return false;
		}
