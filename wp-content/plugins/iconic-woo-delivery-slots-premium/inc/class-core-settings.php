...
class Iconic_WDS_Core_Settings {
	...
	public static function account_getting_started() {
		...
		if ( ! defined( 'ICONIC_WDS_DISABLE_DASH' ) || ( class_exists( 'Iconic_WDS_Core_Licence' ) && ! Iconic_WDS_Core_Licence::has_valid_licence() ) ) {
			return;
		}
			...
			$settings['sections']['licence'] = array(
				...
				'fields'              => array(
					array(
						...
						'type'     => 'custom',
						'default'  => Iconic_WDS_Core_Licence::admin_account_link(),
					),
					array(
						'id'       => 'account',
	...
	public static function add_sidebar() {
		...
		$cross_sells  = Iconic_WDS_Core_Cross_Sells::get_selected_plugins( self::$args['cross_sells'] );
		$show_bundle  = ! Iconic_WDS_Core_Licence::is_bundle() && Iconic_WDS_Core_Licence::get_license_quota() <= 3;
		$is_trial     = Iconic_WDS_Core_Licence::is_trial();
					...
					<?php foreach ( $cross_sells as $cross_sell ) { ?>
						<div class="iconic-product">
							...
							<div class="iconic-product__content">
								<h4 class="iconic-product__title"><a target="_blank" href="<?php echo esc_url( $cross_sell['url'] ); ?>?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckwds&utm_content=cross-sell" target="_blank"><?php echo esc_html( $cross_sell['title'] ); ?></a></h4>
