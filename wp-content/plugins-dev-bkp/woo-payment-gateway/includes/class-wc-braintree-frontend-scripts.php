...
class WC_Braintree_Frontend_Scripts {
	...
	public $global_scripts = array(
		...
		'dropin-v3-ext'      => 'https://js.braintreegateway.com/web/dropin/1.25.0/js/dropin.min.js',
	...
	public function print_footer_scripts() {
		...
		if ( is_checkout() && ! isset( $wp->query_vars['order_pay'] ) && ! is_order_received_page() ) {
			$gateways = array_filter( WC()->payment_gateways()->payment_gateways(), function ( $gateway ) {
				return $gateway instanceof WC_Braintree_Payment_Gateway && $gateway->is_available() && ! $gateway->has_enqueued_scripts( $this, 'checkout' );
			} );
	...
	private function localize_frontend() {
		...
		wp_localize_script( $this->get_handle( 'client-manager' ), 'wc_braintree_client_token', empty( $client_token ) ? null : (array) $client_token );
