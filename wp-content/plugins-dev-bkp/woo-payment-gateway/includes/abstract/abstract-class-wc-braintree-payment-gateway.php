...
abstract class WC_Braintree_Payment_Gateway extends WC_Payment_Gateway {
	...
	public function add_hooks() {
		...
		add_action( 'woocommerce_subscription_payment_method_updated_to_' . $this->id, array( $this, 'subscription_payment_method_updated' ), 10, 1 );
	...
	public function enqueue_mini_cart_scripts( $scripts ) {
		if ( ! wp_script_is( $scripts->get_handle( 'mini-cart' ) ) ) {
			$scripts->enqueue_script( 'mini-cart', $scripts->assets_url( 'js/frontend/mini-cart.js' ), apply_filters( 'wc_braintree_mini_cart_deps', array(
	...
	public function process_payment( $order_id ) {
		...
		try {
			...
			wc_add_notice( sprintf( __( 'There was an error processing your payment. Reason: %1$s', 'woo-payment-gateway' ), $msg ), 'error' );
	...
	public function should_save_payment_method() {
		return ( ! empty( $_POST[ $this->save_method_key ] ) && ! $this->use_saved_method() ) || ( wcs_braintree_active() && ! $this->use_saved_method() &&
		                                                                                           WC_Subscriptions_Cart::cart_contains_subscription() && $this->supports( 'subscriptions' ) );
	}
	...
	public function add_payment_method( $args = array(), $order = false ) {
		...
		try {
			...
			$args     = array_merge(
				array(
					'customerId'         => wc_braintree_get_customer_id( $user_id ),
	...
	private function add_subscription_options( &$args, $order ) {
		$args['options'] = array(
			'submitForSettlement' => apply_filters( 'wc_braintree_subscription_submit_for_settlement', $this->get_option( 'wcs_charge_type' ) === 'capture', $this, $order ),
		);
	...
	public function change_subscription_payment_method( $subscription ) {
		...
		try {
			...
			if ( $response->success ) {
				...
				$subscription->set_payment_method( $this->id );
				$subscription->save();
				$subscription->add_order_note( sprintf( __( 'Payment method changed. New payment method: %1$s. Old Payment method: %2$s', 'woo-payment-gateway' ), $subscription->get_payment_method_title(), $old_payment_method_title ) );
			} else {
	...
	protected function add_order_line_items( &$args, $order, &$items = array() ) {
		if ( $this->is_active( 'line_items' ) ) {
			...
			foreach ( $order->get_items( 'line_item' ) as $item ) {
				...
				if ( 0 < $item->get_subtotal() ) {
					$line_item = array(
						...
						'totalAmount'    => round( $item->get_subtotal(), $this->line_item_validations['totalAmount'] ),
						'unitAmount'     => round( $item->get_subtotal() / $item->get_quantity(), $this->line_item_validations['unitAmount'] ),
						'taxAmount'      => round( $item->get_subtotal_tax(), $this->line_item_validations['taxAmount'] ),
						'discountAmount' => round( $item->get_subtotal() - $item->get_total(), $this->line_item_validations['discountAmount'] )
					);
					if ( ( $unit_tax_amount = round( $item->get_subtotal_tax() / $item->get_quantity(), $this->line_item_validations['unitTaxAmount'] ) ) ) {
					 ...
					 * don't fail.
					 */
					$diff = round( abs( $line_item['totalAmount'] - ( $line_item['unitAmount'] * $line_item['quantity'] ) ), $this->line_item_validations['totalAmount'] );
			...
			foreach ( $order->get_fees() as $fee ) {
				...
				if ( 0 < $fee->get_total() ) {
					$item = array(
						...
						'totalAmount' => round( $fee->get_total(), $this->line_item_validations['totalAmount'] ),
						'unitAmount'  => round( $fee->get_total() / $fee->get_quantity(), $this->line_item_validations['unitAmount'] ),
						'taxAmount'   => round( $fee->get_total_tax(), $this->line_item_validations['taxAmount'] )
					);
					if ( ( $unit_tax_amount = round( $fee->get_total_tax() / $fee->get_quantity(), $this->line_item_validations['unitTaxAmount'] ) ) ) {
			...
			foreach ( $order->get_items( 'coupon' ) as $coupon ) {
				 ...
				 */
				if ( 0 < $coupon->get_discount() ) {
					$item = array(
						'name'        => sprintf( __( 'Coupon - %s', 'woo-payment-gateway' ), $coupon->get_name() ),
						'kind'        => 'credit',
						'productCode' => $coupon->get_id(),
						'quantity'    => intval( $coupon->get_quantity() ),
						'totalAmount' => round( $coupon->get_discount(), $this->line_item_validations['totalAmount'] ),
						'unitAmount'  => round( $coupon->get_discount() / $coupon->get_quantity(), $this->line_item_validations['unitAmount'] ),
						'taxAmount'   => round( $coupon->get_discount_tax(), $this->line_item_validations['taxAmount'] )
					);
					if ( ( $unit_tax_amount = round( $coupon->get_discount_tax() / $coupon->get_quantity(), $this->line_item_validations['unitTaxAmount'] ) ) ) {
						$item['unitTaxAmount'] = $unit_tax_amount;
					}
					$this->format_order_line_item( $item, 'name', $this->line_item_validations['name'] );
			...
			if ( 0 < ( $order->get_discount_total() + $this->order_totals['discount']['subtotal'] ) ) {
				$item = array(
					'name'        => __( 'Discount', 'woo-payment-gateway' ),
					'description' => sprintf( __( 'Discount for order %s', 'woo-payment-gateway' ), $order->get_id() ),
					'kind'        => 'credit',
					'quantity'    => 1,
					'totalAmount' => round( $order->get_discount_total() + $this->order_totals['discount']['subtotal'], $this->line_item_validations['totalAmount'] ),
					'unitAmount'  => round( $order->get_discount_total() + $this->order_totals['discount']['subtotal'], $this->line_item_validations['unitAmount'] ),
					'taxAmount'   => round( $order->get_discount_tax() + $this->order_totals['discount']['tax_total'], $this->line_item_validations['taxAmount'] )
				);
				if ( $item['taxAmount'] ) {
	...
	public function process_pre_order_payment( $order ) {
		...
		try {
			...
			if ( $result->success ) {
				...
				$order->add_order_note( sprintf( __( 'Pre order payment processed. Transaction ID: %1$s. Payment method: %2$s', 'woo-payment-gateway' ), $order->get_transaction_id(), $order->get_payment_method_title() ) );
			} else {
	...
	private function get_payment_section_description() {
		return sprintf( __( 'Increase your conversion rate by offering %1$s on your Product and Cart pages, or at the top of the Checkout page. <br/><strong>Note:</strong> you can control which products display this button by going to the product edit page.', 'woo-payment-gateway' ), $this->get_method_title() );
	}
	...
	public function add_order_general_args( &$args, $order ) {
		...
		$args['taxAmount']         = wc_round_tax_total( $order->get_total_tax() );
		$args['discountAmount']    = $order->get_discount_total();
		$args['shippingAmount']    = $order->get_shipping_total();
	...
	public function output_display_items( $page = '', $data = array() ) {
		...
		if ( in_array( $page, array( 'cart', 'checkout', 'change_payment_method' ) ) ) {
			if ( ! empty( $wp->query_vars['order-pay'] ) || $this->is_change_payment_request() ) {
				...
				if ( 'checkout' === $page && is_add_payment_method_page() ) {
					...
					$data['total'] = strval( WC()->cart->total );
			...
			$data['total']          = strval( round( $product->get_price(), 2 ) );
		 ...
		 * @param WC_Braintree_Payment_Gateway $this
		 */
		$data = apply_filters( 'wc_braintree_output_display_items', $data, $page, $this );
		printf( '<input type="hidden" class="%1$s" data-gateway="%2$s"/>', "woocommerce_{$this->id}_data", htmlspecialchars( wp_json_encode( $data ) ) );
	}
	...
	public function get_display_items_for_cart( $cart, $decimals ) {
		...
		foreach ( $cart->get_cart() as $key => $cart_item ) {
			$total   = round( $incl_tax ? wc_get_price_including_tax( $cart_item['data'], array( 'qty' => $cart_item['quantity'] ) ) : wc_get_price_excluding_tax( $cart_item['data'], array( 'qty' => $cart_item['quantity'], ) ), $decimals );
		...
		if ( WC()->cart->needs_shipping() && WC()->cart->show_shipping() ) {
			$total   = round( $incl_tax ? WC()->cart->shipping_total + WC()->cart->shipping_tax_total : WC()->cart->shipping_total, $decimals );
			$label   = __( 'Shipping', 'woocommerce' );
			$items[] = $this->get_display_item_for_cart( $total, $label, 'shipping', $cart, ...$args );
		}
		foreach ( WC()->cart->get_fees() as $fee ) {
			$total   = round( $incl_tax ? $fee->total + $fee->tax : $fee->total, $decimals );
			$label   = esc_html( $fee->name );
			$items[] = $this->get_display_item_for_cart( $total, $label, 'fee', $cart, ...$args );
		}
		if ( 0 < WC()->cart->discount_cart ) {
			$total   = round( - 1 * abs( $incl_tax ? WC()->cart->discount_cart + WC()->cart->discount_cart_tax : WC()->cart->discount_cart ), $decimals );
		...
		if ( wc_tax_enabled() && ! $incl_tax ) {
			$total   = round( WC()->cart->get_taxes_total(), $decimals );
	...
	public function get_display_items_for_order( $order, $decimals = 2 ) {
		...
		$items = array();
		foreach ( $order->get_items( 'line_item' ) as $item ) {
			$total   = round( $item->get_subtotal(), $decimals );
		...
		if ( 0 < $order->get_shipping_total() ) {
			$total   = round( $order->get_shipping_total(), $decimals );
	...
	/**
	 * @param float $total the item total
			...
			foreach ( $recurring_cart->get_cart() as $cart_item ) {
				if ( ( wcs_braintree_active() && WC_Subscriptions_Product::get_trial_length( $cart_item['data'] ) > 0 ) ||
					...
					if ( $recurring_cart->needs_shipping() ) {
						...
						$total   = round( $incl_tax ? $recurring_cart->shipping_total + $recurring_cart->shipping_tax_total : $recurring_cart->shipping_total, $decimals );
					...
					if ( 0 < $recurring_cart->discount_cart ) {
						...
						$total   = round( - 1 * ( $incl_tax ? $recurring_cart->discount_cart + $recurring_cart->discount_cart_tax : $recurring_cart->discount_cart ), $decimals );
					...
					if ( wc_tax_enabled() && ! $incl_tax ) {
						...
						$total   = round( $recurring_cart->get_taxes_total(), $decimals );
					...
					$total   = round( $recurring_cart->total, $decimals );
					...
					$total   = round( - 1 * $recurring_cart->total, $decimals );
					$items[] = $this->get_display_item_for_cart( $total, $label, 'subtotal', $recurring_cart );
				} else {
					$label   = sprintf( _n( 'Recurring Total', 'Recurring Total: %s', $index, 'woo-payment-gateway' ), $index );
					$total   = round( $recurring_cart->total, $decimals );
	 ...
	 * @since 3.2.17
	 */
	protected function do_post_payment_processing( $order ) {
