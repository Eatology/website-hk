...
function wc_braintree_save_cc_enabled() {
	...
	return apply_filters(
		...
		is_user_logged_in() && is_checkout() &&
		WC()->payment_gateways()->payment_gateways()['braintree_cc']->is_active( 'save_card_enabled' )
	);
}
...
function wc_braintree_get_order_for_payment_id( $payment_id ) {
	global $wpdb;
	$result = $wpdb->get_row( $wpdb->prepare( "SELECT ID FROM $wpdb->posts as posts INNER JOIN $wpdb->postmeta as postmeta ON postmeta.post_id = posts.ID WHERE posts.post_type = 'shop_order' AND postmeta.meta_key = '_wc_braintree_payment_id' AND postmeta.meta_value = %s", $payment_id ) );
	if ( $result ) {
		$order_id = $result->ID;
