(function ($, wc_braintree) {
    ...
    PayPal.prototype.handle_tokenize_response = function (response) {
        if (this.needs_shipping()) {
            // set nonce and other data just in case other plugins prevent updated_checkout form firing
    ...
    PayPal.prototype.on_payment_method_received = function () {
        ...
        if (this.needs_shipping() && this.processingBannerCheckout) {
            this.scroll_to_place_order();
        } else {
            if ((!this.needs_shipping() || (this.needs_shipping() && $('[name^="shipping_method"]').length < 2)) && this.validate_checkout_fields()) {
