(function ($) {
        ...
        wc_braintree.create_instance = function (clazz) {
            ...
            instance.create_instance(this.clientInstance, wc_braintree_client_token);
        ...
        wc_braintree.triggerClientUpdate = function (client) {
            ...
            this.gateways.forEach(function (instance) {
                instance.create_instance(this.clientInstance, wc_braintree_client_token, this.merchant_account);
            }.bind(this))
        }
        ...
        wc_braintree.BaseGateway.prototype.get_address_object = function (prefix) {
            var address = {};
            this.get_address_mappings().forEach(function (k) {
        ...
        wc_braintree.CheckoutGateway.prototype.hasOrderReviewParams = function () {
            ...
            if (matches && matches.length > 1) {
                try {
                    var obj = JSON.parse(window.atob(matches[1]));
            ...
            return new Promise(function (resolve, reject) {
                ...
                if (this.config.paypal || !this.config.paypalEnabled) {
                    if (!this.config.paypalEnabled || this.config.paypal.clientId === null) {
                        ...
                        this.submit_error('No Braintree merchant account was found for currency %s. Ensure you have imported'.replace('%s', this.get_currency()) +
        ...
        wc_braintree.PayPal.prototype.get_button_container = function () {
            ...
            if (!$parent.find('.wc-braintree-paypal-button-container').length) {
                $parent.append('<div class="wc-braintree-paypal-button-container"></div>');
            }
        ...
        wc_braintree.CreditCard.prototype.process_3dsecure = function (response, isVaulted) {
            if (this.threeDSecureInstance) {
                ...
                this.threeDSecureInstance.verifyCard({
                    amount: this.is_checkout_page() ? this.get_order_total() : this.get_total(),
                    ...
                    billingAddress: {
                        ...
                        streetAddress: this.fields.get('billing_address_1', ''),
                        extendedAddress: this.fields.get('billing_address_2', ''), // available
                        locality: this.fields.get('billing_city', ''),
                    ...
                    additionalInformation: this.needs_shipping() ? {
                        ...
                        shippingAddress: {
                            ...
                            extendedAddress: this.fields.get(prefix + '_address_2', ''),
                ...
                if (this.can_initialize_applepay()) {
                    braintree.applePay.create({
                        ...
                        promise.then(function (canMakePayments) {
                            // resolve regardless of canMakePayments because customer can add a payment method
                            resolve();
                        });
                    }.bind(this));
                } else {
