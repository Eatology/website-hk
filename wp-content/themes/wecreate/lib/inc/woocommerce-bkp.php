...
if(!function_exists('get_holidays_count'))
{
    function get_holidays_count($paramDate, $start_type)
    {
        ...
        $earliestDateAvailable = new DateTime($paramDate);

        $earliestDAv  = date_format($earliestDateAvailable, $new_dte_format);

        $days_to_add = 0;
        foreach ($holidays as $holiday) {
            if (in_array($earliestDAv, $holiday)) {
                $sdate = $holiday['date'];
                ...
                $days_to_add = (int)$diff->format("%d") + $minimum_day;
                }
        }
...
if(!function_exists('getEarliestAvailableDates'))
{
    function getEarliestAvailableDates()
    {
        ...
        // if so, then add one more day, as sunday is not a working day
        ...
        if($holidays_count == 1)
        { 
            $end = date('Y-m-d', strtotime($end . "+" . $holidays_count . " day"));
            ...
            if (($day_of_end_date == 'Sat' && $daysVariation == 5) || ($day_of_end_date == 'Sun' && $daysVariation == 5)) {
                $end = date('Y-m-d', strtotime($end . "+2 days"));
            }
            // for 6 day meal plan
...
//         echo "<pre>";
  //       print_r($daterange);
        ...
        foreach ($daterange as $date) {
			...
			if (($date->format('D') == 'Sat' && $max_day == 5) || ($date->format('D') == 'Sun' && $max_day == 5))
            {
        ...
        if ($sunday) {
            // three days after
            $days = $holidays_count + 3;
        } else if ($weekendNFiveDay){
            // two days after
            $days = $holidays_count + 4;
        }
        else {
            // two days after
            $days = $holidays_count + 2;
        }

        // echo "Days" . $days;
        // correct till now

        $earliestDate = date('Y-m-d', strtotime("+".$days." days"));
...
//         echo "<pre>";
  //       print_r($earliestDate);
		...
		if (($checkSundayTheDay->format('D') == 'Sat' && $max_day == 5) || ($checkSundayTheDay->format('D') == 'Sun' && $max_day == 5)) {
			$earliestDate = date('Y-m-d', strtotime($earliestDate . "+2 days"));
		}	
