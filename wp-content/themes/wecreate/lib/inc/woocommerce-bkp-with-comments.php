...
if(!function_exists('get_holidays_count'))
{
    function get_holidays_count($paramDate, $start_type)
    {
        ...
        foreach ($holidays as $holiday) {
            ...
            // return $days_to_add + $minimum_day;
            return $days_to_add;
    }
}
...
if(!function_exists('getEarliestAvailableDates'))
{
    function getEarliestAvailableDates()
    {
        ...
        {
            // echo "<br> Before adding holidays : ";
            // print_r($end);

            $holidays_count = get_holidays_count('now', 'today');
            $end = date('Y-m-d', strtotime($end . " +" . $holidays_count . " days"));
            // echo "<br>Holidays Count Prev " . $holidays_count;
            // echo "<br> After adding holidays : ";
            // print_r($end);
        }
        ...
        {
            // echo "<br> Before adding holidays Two: ";
            // print_r($end);

            $holidays_count = get_holidays_count($tomrow, 'today');
            $end = date('Y-m-d', strtotime($end . " +" . $holidays_count . " days"));
            // echo "<br>Holidays Count New " . $holidays_count;
            // echo "<br> After adding holidays Two - End Date: ";
            // print_r($end);
        }
        ...
        $days = 0;
        if ($weekendNFiveDay){
            $days = 1; // add one more day for excaping Sat on Five day meal plan order
        }
        // else if ($sunday) {
        //     // echo "else if";
        //     // three days after
        ...
        if(isEarliestDayAHoliday($earliestDate))
        {
            // get holidays count
            $days_to_add = get_holidays_count($earliestDate, 'future');
            // $days_to_add = $days_to_add - 1; // subtracting 1 day, because the $minimum_day shouldnot be added twice here in this function
            $str = " +".$days_to_add."day";
            $earliestDate = date('Y-m-d', strtotime($earliestDate . $str));
        }
