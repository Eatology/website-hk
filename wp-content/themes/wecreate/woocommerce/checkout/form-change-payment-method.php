...
<form id="order_review" method="post">
	<div class="rounded-bubble">
		...
		<h2><?php esc_html_e( 'Payment Details', 'woocommerce-subscriptions' ); ?></h2>
		<table class="shop_table">
			...
			<?php endforeach; ?>
				<tr>
					<td colspan="3" class="change-line"></td>
				</tr>
			</tbody>
		</table>

		<div class="change-totals-table">
			<?php foreach ( $subscription->get_order_item_totals() as $total ) : ?>
				<div class="total-wrapper <?php echo strtolower(sanitize_html_class( $total['label'] )); ?>">
					<span class="total-title"><?php echo esc_html( $total['label'] ); ?></span>
					<?php echo wp_kses_post( $total['value'] ); ?>
				</div>
			<?php endforeach; ?>
		</div>

		<div class="change-payment-line"></div>

		<h3><?php esc_html_e( 'Billing Details', 'woocommerce-subscriptions' ); ?></h3>
				...
				<ul class="payment_methods methods">
					...
					foreach ( $available_gateways as $gateway ) :
						...
						<li class="wc_payment_method payment_method_<?php echo esc_attr( $gateway->id ); ?>">
							<label for="payment_method_<?php echo esc_attr( $gateway->id ); ?>" class="label-container">
