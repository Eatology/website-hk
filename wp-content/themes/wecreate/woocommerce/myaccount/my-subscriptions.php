...
<h2>Subscriptions</h2>
	...
	<thead>
		...
		<tr>
			<th class="subscription-id order-number woocommerce-orders-table__header woocommerce-orders-table__header-order-number woocommerce-orders-table__header-subscription-id"><span class="nobr"><?php esc_html_e( 'No.', 'woocommerce-subscriptions' ); ?></span></th>
			...
			<th class="subscription-status order-status woocommerce-orders-table__header woocommerce-orders-table__header-order-status woocommerce-orders-table__header-subscription-status"><span class="nobr"><?php esc_html_e( 'Service Period', 'woocommerce-subscriptions' ); ?></span></th>
			...
			<th class="subscription-actions order-actions woocommerce-orders-table__header woocommerce-orders-table__header-order-actions woocommerce-orders-table__header-subscription-actions"><span class="nobr"><?php echo esc_html_x( 'Action', 'table heading', 'woocommerce-subscriptions' ); ?></span></th>
		</tr>
	</thead>
	...
	<?php foreach ( $subscriptions as $subscription_id => $subscription ) : ?>
		<tr class="order woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $subscription->get_status() ); ?>">
			...
			<td class="subscription-service order-status woocommerce-orders-table__cell woocommerce-orders-table__cell-subscription-status woocommerce-orders-table__cell-order-status" data-title="<?php esc_attr_e( 'Service Period', 'woocommerce-subscriptions' ); ?>">
				<?php 
					$start_date = strtotime(esc_attr( $subscription->get_date_to_display( 'start_date' )));
					$start_date_formatted = date( "j M Y", $start_date);
					echo  $start_date_formatted; 
				?> - 
				<?php 
					$end_date = strtotime(esc_attr( $subscription->get_date_to_display( 'end_date' )));
					$end_date_formatted = date( "j M Y", $end_date);
